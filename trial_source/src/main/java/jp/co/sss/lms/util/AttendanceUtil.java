...
public class AttendanceUtil {
	...
	private AttendanceStatusEnum getStatus(
		...
		if (trainingStartTime != null
				&& trainingStartTime.isNotBlank()) {

			isLate =
					(trainingStartTime.compareTo(workStartTime) > 0);
		}

		// 定時より1分以上早く退社していたら早退(＝はセーフ)
		if (trainingEndTime != null
				&& trainingEndTime.isNotBlank()) {

			isEarly =
					(trainingEndTime.compareTo(workEndTime) < 0);
		}
	...
	public Date getTrainingDate() {
		...
		try {
			...
			// DateUtil#toStringとparseは同様のフォーマットを使用しているため、
			// 起こりえないエラー
	...
	public Integer getHour(String time) {
		...
		TrainingTime trainingTime =
				new TrainingTime(time);

		return trainingTime.getHour();
	}
	...
	public boolean isWorkDay(
		...
		Integer count =
				mSectionMapper.getSectionCountByCourseId(
						courseId,
						trainingDate);
