...
public class StudentAttendanceService {
		...
		for (AttendanceManagementDto dto : attendanceManagementDtoList) {
			...
			if (dto.getBlankTime() != null) {
				...
				dto.setBlankTimeValue(
						String.valueOf(blankTime));
			}

			// 遅刻早退区分判定
			AttendanceStatusEnum statusEnum =
					AttendanceStatusEnum.getEnum(
							dto.getStatus());
	...
	public Boolean notEnterCheck() throws ParseException {
		...
		Integer notEnterCount =
				tStudentAttendanceMapper.notEnterCount(
	...
	public String punchCheck(Short attendanceType) {
		...
		if (!loginUserUtil.isStudent()) {
			...
			return messageUtil.getMessage(
					Constants.VALID_KEY_AUTHORIZATION);
		}
		...
		// 登録情報チェック
		TStudentAttendance tStudentAttendance =
				tStudentAttendanceMapper
						.findByLmsUserIdAndTrainingDate(
								loginUserDto.getLmsUserId(),
								trainingDate,
								Constants.DB_FLG_FALSE);
	...
	public String setPunchIn() {
		...
		if (tStudentAttendance == null) {
			...
			// 登録処理
			tStudentAttendance =
					new TStudentAttendance();

			tStudentAttendance.setLmsUserId(
					loginUserDto.getLmsUserId());
			...
			tStudentAttendance.setAccountId(
			...
			tStudentAttendance.setFirstCreateDate(
					date);

			tStudentAttendance.setLastModifiedUser(
					loginUserDto.getLmsUserId());

			tStudentAttendance.setLastModifiedDate(
					date);
	...
	public AttendanceForm setAttendanceForm(
		...
		attendanceForm.setAttendanceList(
				new ArrayList<DailyAttendanceForm>());

		attendanceForm.setLmsUserId(
				loginUserDto.getLmsUserId());

		attendanceForm.setUserName(
				loginUserDto.getUserName());

		attendanceForm.setLeaveFlg(
				loginUserDto.getLeaveFlg());
		...
		for (AttendanceManagementDto attendanceManagementDto
			...
			if (attendanceManagementDto.getBlankTime()
					!= null) {
			...
			dailyAttendanceForm.setSectionName(
					attendanceManagementDto
							.getSectionName());

			dailyAttendanceForm.setIsToday(
					attendanceManagementDto
							.getIsToday());
			...
			dailyAttendanceForm.setStatusDispName(
					attendanceManagementDto
							.getStatusDispName());

			attendanceForm
					.getAttendanceList()
					.add(dailyAttendanceForm);
		}
	...
	public void formatConversion(
		...
		for (DailyAttendanceForm dailyAttendanceForm
			...
			Integer startHour =
					dailyAttendanceForm
							.getTrainingStartTimeHour();

			Integer startMinute =
					dailyAttendanceForm
							.getTrainingStartTimeMinute();
			...
			Integer endHour =
					dailyAttendanceForm
							.getTrainingEndTimeHour();

			Integer endMinute =
					dailyAttendanceForm
							.getTrainingEndTimeMinute();
	...
	public String update(
		...
		for (DailyAttendanceForm dailyAttendanceForm
			...
			// 現在の勤怠情報リストのうち、
			// 研修日が同じものを更新用エンティティで上書き
			for (TStudentAttendance entity
					: tStudentAttendanceList) {

				if (entity.getTrainingDate()
						.equals(
								tStudentAttendance
										.getTrainingDate())) {
			...
			// 登録用Listへ追加
			tStudentAttendanceList.add(
					tStudentAttendance);
		}

		// 登録・更新処理
		for (TStudentAttendance tStudentAttendance
				: tStudentAttendanceList) {

			if (tStudentAttendance
					.getStudentAttendanceId() == null) {
